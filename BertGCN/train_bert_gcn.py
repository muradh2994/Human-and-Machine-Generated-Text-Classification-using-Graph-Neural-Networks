...
parser.add_argument('--dataset', default='20ng', choices=['20ng', 'R8', 'R52', 'ohsumed', 'mr','humanvsai'])
...
def log_training_results(trainer):
    ...
    metrics = evaluator.state.metrics
    test_acc, test_nll,test_pr,test_rc = metrics["acc"], metrics["nll"],metrics["pr"],metrics["rc"]
    logger.info(
        "Epoch: {}  Train acc: {:.4f} loss: {:.4f}  Val acc: {:.4f} loss: {:.4f}  Test acc: {:.4f} loss: {:.4f} precision: {:.4f} recall: {:.4f}"
        .format(trainer.state.epoch, train_acc, train_nll, val_acc, val_nll, test_acc, test_nll, test_pr, test_rc)
    )
    
    epoch = trainer.state.epoch
    global best_precision , best_recall, e
    global train_accuracy, val_accuracy, train_loss, val_loss
    
    best_precision = 0. if epoch == 1 else best_precision
    best_recall = 1 if epoch == 1 else best_recall
    
    e.append(epoch)
    train_accuracy.append(np.round(train_acc,2))
    val_accuracy.append(np.round(val_acc,2))
    train_loss.append(np.round(train_nll,2))
    val_loss.append(np.round(val_nll,2))
    
    #precision_val.append(test_pr.numpy())
    #recall_val.append(test_rc.numpy())
...
F1_score = ( best_precision* best_recall * 2 / (best_precision + best_recall)) 
print("Precision :", np.round(best_precision,2))
print("Recall : ", np.round(best_recall,2))
print("F1 Score :", np.round(F1_score,2))

fig = plt.figure(1)	#identifies the figure 
plt.title("Accuracy vs epoch", fontsize='16')	#title
plt.plot(e, train_accuracy, label= 'Train')	#plot the points
plt.plot(e, val_accuracy, label= 'Validation')
...
plt.savefig('Accuracy_bert.png')	#saves the figure in the present directory
plt.show()

fig = plt.figure(2)	#identifies the figure 
plt.title("Loss vs epoch", fontsize='16')	#title
plt.plot(e, train_loss, label= 'Train')	#plot the points
plt.plot(e, val_loss, label= 'Validation')
plt.xlabel("Epoch",fontsize='13')	#adds a label in the x axis
plt.ylabel("Loss",fontsize='13')	#adds a label in the y axis
plt.legend(loc='best')	#creates a legend to identify the plot
plt.savefig('Loss_bert.png')	#saves the figure in the present directory
plt.show()